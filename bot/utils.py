...
def load_processed() -> Set[str]:
    if os.path.exists(CONFIG.PROCESSED_DB.value):
        try:
            with open(CONFIG.PROCESSED_DB.value, "r") as f:
...
def save_processed(filename: str) -> None:
    ...
    if filename not in data:
        ...
        with open(CONFIG.PROCESSED_DB.value, "w") as f:
            json.dump(data, f)
...
def load_explorer_cache() -> Dict[str, Any]:
    if os.path.exists(CONFIG.EXPLORER_CACHE_DB.value):
        try:
            with open(CONFIG.EXPLORER_CACHE_DB.value, "r") as f:
                return json.load(f)
        except:
            return {}
    return {}
...
def save_explorer_cache(url: str, files: list) -> None:
    ...
    with open(CONFIG.EXPLORER_CACHE_DB.value, "w") as f:
        json.dump(cache, f)
